...
comment on column Prof.name is '교수명';
comment on column Prof.likecnt is '좋아요수';
...
-- 4번문제) 자신의 매니저보다 더 많은 급여를 받는 사람 목록 *********************
select * from Employees;
...
-- 5번문제) job title이 sales represntative인 직원 중에서, 급여가 9,000 ~ 10,000인 직원들의 이름과 급여를 출력하시오. **************
select * from Employees;
select * from Jobs;
...
select concat(concat(e.first_name, ' '), e.last_name) as "직원 이름" , e.salary "급여"
from Employees e  where e.job_id = 'SA_REP' and e.salary between 9000 and 10000 order by salary desc;


-- 6번문제) 각 직급별로 급여의 총합을 구하고자 한다.급여 총합이 가장 높은 직급순으로 급여 총합을 출력하시오. (단, 급여총합이 30,000 이상인 직급만 출력할 것)
...
-- 7번문제) 각 도시별 평균 연봉(급여)가 높은순으로 상위 3개 도시만 출력하시오.
select * from locations;
...
-- 8번문제) 직책(Job Title)이 Sales Manager인 사원들의 입사년도(hire_date)별 평균 급여를 출력하시오. (출력 시 년도를 기준으로 오름차순 정렬하시오.)
select *
  from Employees;
...
/* 13번문제) EMPLOYEES 테이블에서 급여를 많이 받는 순서대로 조회했을 때
   6번째부터 10 번째까지 5명의 last_name, first_name, salary를 조회하는
   sql문장을 작성하시오. */

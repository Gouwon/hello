...
class BodyRequest(ISerializable):
    """
    파일 전송 요청 메세지(0x01)에 사용할 본문 클래스입니다.
    FILESIZE와 FILENAME 데이터 속성을 가집니다.
    ...
    def __init__(self, buffer):
        if buffer != None:
            ...
            # 1 unsigend long long, N character
            self.struct_fmt = str.format('=Q{0}s', slen - 8)
            self.struct_len = struct.calcsize(self.struct_fmt)

            # unsigned long long의 크기
            if slen > 4:
                slen = slen - 4
            else:
                slen = 0
...
class BodyResponse(ISerializable):
    """
    파일 전송 요청에 대한 응답 메세지(0x02)에 사용할 본문 클래스입니다.
    요청 메세지의 MSGID와 수락 여부를 나타내는 RESPONSE 데이터 속성을 가집니다.
    ...
    def __init__(self, buffer):
        ...
        if buffer != None:
            ...
            self.MSGID, self.RESPONSE = unpacked
        else:
            self.MSGID = 0
...
class BodyData(ISerializable):
    """
    실제 파일을 전송하는 메세지(0x03)에 사용할 본문 클래스입니다.
    앞서 프로토콜 정의에서 이야기했던 것처럼 DATA 필드만 갖고 있습니다.
...
class BodyResult(ISerializable):
    """
    파일 전송 결과 메세지(0x04)에 사용할 본문 클래스입니다.
    요청 메세지의 MSGID와 성공 여부를 나타내는 RESULT 데이터 속성을 가집니다.
    ...
    def __init__(self, buffer):
        ...
        if buffer:
            unpacked = struct.unpack(self.struct_fmt, buffer)
            self.MSGID, self.RESULT = unpacked
        else:
            self.MSGID = 0
